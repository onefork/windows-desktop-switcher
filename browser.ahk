...
; Last Mod:           2016-05-19
; Source:             https://www.autohotkey.com/boards/viewtopic.php?t=3702
...
GetActiveBrowserURL() {
    ...
    If sClass In % ModernBrowsers
        Return GetBrowserURL_ACC(nWindow)
...
GetBrowserURL_ACC(nWindow) {
    ...
    ; }
    sURL := GetAddressBarUrl(Acc_ObjectFromWindow(nWindow))
    ...
    ;     }
    ; }
    ; If ((sURL != "") and (not InStr(sURL,"://"))) ; Modern browsers omit "http://"
    ;     sURL := "http://" sURL
    If (sURL == "")
...
GetAddressBarUrl(accObj) {
    Try If ((accObj.accRole(0) == 42) and (url := accObj.accValue(0))) {
        if (not InStr(url, "://"))
            url := "http://" url ; Modern browsers omit "http://"
        url := getUrl(url)
        if (url)
            Return url
    }
    For nChild, accChild in Acc_Children(accObj)
        If (url := GetAddressBarUrl(accChild))
            Return url
}
...
Acc_Children(Acc) {
    ...
    Else {
        try { ; catch error if target hwnd become unavailable during the process (closed)
